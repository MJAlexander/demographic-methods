...
So the proportions associated with departures are probabilities of moving...

Set up matrix as Vic, Tas to Vic, Tas, Vic to Tas. Assuming to return migrant, there are eight separate matrices. The native ones are as before with extra multipliers based on probability of migration
...
```{r}
log(Re(eigen(M)$values[1]))/5

plot(Re(eigen(M)$vectors[,1]/sum(eigen(M)$vectors[,1])))
```
