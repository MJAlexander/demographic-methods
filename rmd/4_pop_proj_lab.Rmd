...
2. Calculate the NRR for each of the countries. Confirm that these are one high and one low, based on NRR values. 
...
7. Plot the total population size over time and the proportion in each age group

## Eigendecompositions

8. Do an eigendecomposition on `A` using `eigen(A)`. What is the intrinsic growth rate? What is the implied stable population age structure? 
